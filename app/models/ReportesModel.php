...
class ReportesModel {
    ...
    public static function getAsistenciaMensualPorGrupo(PDO $pdo, string $inicio, string $fin): array {
        $sql = "SELECT
                    ...
                    -- Faltas Reales (Ausencias puras)
                    SUM(CASE WHEN status = 'Ausente' THEN 1 ELSE 0 END) as faltas_reales
    ...
    public static function getResumenMensualGlobal(PDO $pdo, string $inicio, string $fin): array {
        $sql = "SELECT
                    ...
                    SUM(CASE WHEN status = 'Ausente' THEN 1 ELSE 0 END) as total_faltas_reales
                FROM asistencia_clases
                WHERE fecha BETWEEN ? AND ?
    ...
    /**
     * Obtiene el conteo de 'Faltas Reales' (días con 100% de ausencia) para toda la academia.
     * Se define como días donde el alumno tuvo 'Ausente' en TODAS sus clases del día.
     */
    public static function getFaltasRealesAcademia(PDO $pdo, string $inicio, string $fin): int {
        $sql = "SELECT COUNT(*) 
                FROM (
                    SELECT alumno_nombre, fecha
                    FROM asistencia_clases
                    WHERE fecha BETWEEN ? AND ?
                    GROUP BY alumno_nombre, fecha
                    HAVING (SUM(CASE WHEN status = 'Ausente' THEN 1 ELSE 0 END) / CAST(COUNT(*) AS FLOAT)) * 100 >= 100
                ) as dias_criticos";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$inicio, $fin]);
    ...
    /**
     * Obtiene faltas reales por mes (días 100% ausencia).
     */
    public static function getFaltasRealesMensual(PDO $pdo, string $inicio, string $fin): array {
        $sql = "SELECT 
                    mes_key,
                    mes_num,
                    COUNT(*) as total_faltas_reales
                FROM (
                    SELECT 
                        alumno_nombre, 
                        fecha,

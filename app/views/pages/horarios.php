<?php
/**
 * app/views/pages/horarios.php
 * Gestor de Horarios — v2. Conectado a PostgreSQL via api/horarios.php.
 * Mejoras: fix editar, edificio en aulas, importar materias en bulk,
 *          sync docentes/grupos desde asistencia_clases, carga de alumnos.
 */
?>
<div class="bg-slate-50 -m-8 min-h-screen flex flex-col" id="horarios-app">

    <!-- Header -->
    <div class="bg-white border-b border-slate-200 px-8 py-5 flex items-center justify-between shrink-0">
        <div class="flex items-center gap-4">
            <div class="w-10 h-10 rounded-xl bg-blue-600 flex items-center justify-center text-white">
                <span class="material-symbols-outlined">calendar_month</span>
            </div>
            <div>
                <h1 class="text-xl font-black text-slate-900 tracking-tight">Gestor de Horarios</h1>
                <p class="text-xs font-bold text-slate-400 uppercase tracking-widest" id="hor-subtitle">Cargando...</p>
            </div>
        </div>
        <div class="flex items-center gap-3">
            <div id="planning-indicator" class="hidden flex items-center gap-4 bg-amber-50 border border-amber-200 px-4 py-2 rounded-xl shadow-sm">
                <div class="flex flex-col">
                    <span class="text-[9px] font-black text-amber-600 uppercase leading-none">Modo Planeación Activo</span>
                    <span id="planning-period-label" class="text-xs font-bold text-amber-900 leading-tight">Siguiente Periodo</span>
                </div>
                <div class="h-8 w-px bg-amber-200 mx-1"></div>
                <button onclick="horarios.copyCurrentPeriod()" class="flex items-center gap-1.5 bg-amber-100 hover:bg-amber-200 text-amber-700 px-3 py-1.5 rounded-lg font-black text-[10px] transition-all uppercase tracking-tight">
                    <span class="material-symbols-outlined text-[16px]">content_copy</span>
                    Clonar Horario Actual
                </button>
                <button onclick="horarios.togglePlanning(false)" class="text-amber-400 hover:text-amber-600 ml-2" title="Salir de Planeación">
                    <span class="material-symbols-outlined text-[20px]">cancel</span>
                </button>
            </div>
            <button onclick="horarios.openModal('config')" class="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl font-bold text-xs transition-all shadow-sm shadow-blue-200">
    ...
    <!-- Tabs Navigation -->
    <div class="bg-white border-b border-slate-100 px-8 flex gap-1 shrink-0 overflow-x-auto">
        <?php
        $tabs = [
            ['id' => 'grid',     'icon' => 'grid_view',     'label' => 'Matriz Semanal'],
            ['id' => 'docentes', 'icon' => 'badge',         'label' => 'Docentes'],
            ['id' => 'materias', 'icon' => 'book',          'label' => 'Materias'],
            ['id' => 'grupos',   'icon' => 'groups',        'label' => 'Grupos'],
            ['id' => 'analisis', 'icon' => 'analytics',     'label' => 'Análisis'],
            ['id' => 'alumnos',  'icon' => 'person',        'label' => 'Alumnos'],
            ['id' => 'aulas',    'icon' => 'meeting_room',  'label' => 'Aulas'],
            ['id' => 'proyeccion', 'icon' => 'query_stats',   'label' => 'Proyección'],
        ];
        foreach ($tabs as $tab): ?>
        <button onclick="horarios.switchTab('<?= $tab['id'] ?>')"
                id="tab-<?= $tab['id'] ?>"
                class="flex items-center gap-2 px-4 py-3.5 text-xs font-bold text-slate-400 hover:text-blue-600 border-b-2 border-transparent transition-all hor-tab whitespace-nowrap">
    ...
    <div class="flex-1 p-8 overflow-y-auto">
        ...
        <!-- ── MATRIZ / GRID ─────────────────────────────────────────────── -->
        <div id="panel-grid" class="hor-panel">
            <div class="flex flex-wrap gap-4 mb-6">
                <div class="flex items-center gap-2 bg-white rounded-xl border border-slate-200 shadow-sm px-4 py-2">
                    <span class="material-symbols-outlined text-slate-400 text-[16px]">schedule</span>
                    <select id="filter-turno-view" onchange="horarios.renderGrid()" class="text-sm font-bold text-slate-700 bg-transparent border-none outline-none">
                        <option value="matutino">Matutino (7-14)</option>
                        <option value="vespertino">Vespertino (14-21)</option>
                        <option value="completo">Día Completo</option>
                    </select>
                </div>
                <div class="flex items-center gap-2 bg-white rounded-xl border border-slate-200 shadow-sm px-4 py-2">
                    <span class="material-symbols-outlined text-slate-400 text-[16px]">groups</span>
                    <select id="filter-grupo" onchange="horarios.renderGrid()" class="text-sm font-bold text-slate-700 bg-transparent border-none outline-none"></select>
                </div>
                <div class="flex items-center gap-2 bg-white rounded-xl border border-slate-200 shadow-sm px-4 py-2">
                    <span class="material-symbols-outlined text-slate-400 text-[16px]">badge</span>
                    <select id="filter-docente" onchange="horarios.renderGrid()" class="text-sm font-bold text-slate-700 bg-transparent border-none outline-none"></select>
                </div>
                <div class="flex gap-2 ml-auto">
                    <button onclick="window.print()" class="flex items-center gap-2 bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 py-2 rounded-xl font-bold text-xs transition-all no-print">
                        <span class="material-symbols-outlined text-[16px]">picture_as_pdf</span> PDF
                    </button>
                    <button onclick="horarios.generateAutomatico()" class="flex items-center gap-2 bg-blue-50 text-blue-600 hover:bg-blue-100 px-4 py-2 rounded-xl font-bold text-xs transition-all no-print">
                        <span class="material-symbols-outlined text-[16px]">auto_fix_high</span> Generación Automática
                    </button>
                    <button onclick="horarios.exportExcel()" class="flex items-center gap-2 bg-emerald-50 text-emerald-600 hover:bg-emerald-100 px-4 py-2 rounded-xl font-bold text-xs transition-all no-print">
                        <span class="material-symbols-outlined text-[16px]">table_view</span> Excel
                    </button>
                    <button onclick="horarios.openModal('clase')" class="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl font-bold text-xs transition-all shadow-sm shadow-blue-200">
                        <span class="material-symbols-outlined text-[16px]">add</span> Agregar Clase
                    </button>
                </div>
            </div>
            <div class="flex gap-4 items-start">
                <div class="flex-1 bg-white rounded-2xl border border-slate-100 shadow-sm overflow-auto min-w-0">
                    <div class="px-6 py-2 bg-slate-50 border-b border-slate-100 flex justify-between items-center no-print">
                        <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest" id="debug-info">Cargando datos...</span>
                    </div>
                    <div id="schedule-grid" class="grid-container" style="display:grid"></div>
                </div>
                
                <!-- Sidebar de Materias Sin Asignar -->
                <div id="unassigned-sidebar" class="hidden w-56 shrink-0 bg-slate-50 rounded-2xl border border-rose-100 shadow-inner flex flex-col max-h-[70vh] no-print">
                    <div class="p-4 border-b border-rose-100 bg-white rounded-t-2xl">
                        <h3 class="font-black text-rose-600 text-[13px] flex items-center gap-1.5 uppercase tracking-wide">
                            <span class="material-symbols-outlined text-[16px]">person_off</span>
                            Falta Docente
                        </h3>
                        <p class="text-[9px] text-slate-500 font-bold leading-tight mt-1">Arrastra estas clases hacia el horario de un profesor.</p>
                    </div>
                    <div id="unassigned-list" class="p-3 overflow-y-auto flex-1 space-y-2 relative"></div>
                </div>
            </div>
        </div>

        <!-- ── DOCENTES ───────────────────────────────────────────────────── -->
        <div id="panel-docentes" class="hor-panel hidden">
            <div class="flex justify-between items-center mb-6 flex-wrap gap-3">
                <h2 class="text-lg font-black text-slate-900 uppercase tracking-tight">Gestión de Docentes</h2>
                <div class="flex gap-2">
                    <div class="flex items-center gap-2 bg-blue-50 text-blue-700 px-4 py-2 rounded-xl font-bold text-xs border border-blue-100 italic">
                        <span class="material-symbols-outlined text-[16px]">verified_user</span> Gestión centralizada en Docentes Autorizados
                    </div>
                </div>
            </div>
            <p class="text-xs text-slate-400 font-bold mb-4">💡 Los docentes se sincronizan automáticamente desde el panel principal de <b>Docentes Autorizados</b>. Aquí puedes configurar su disponibilidad, carga horaria y colores para el horario.</p>
            <div id="docentes-list" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4"></div>
        </div>

        <!-- ── MATERIAS ───────────────────────────────────────────────────── -->
        <div id="panel-materias" class="hor-panel hidden">
            <div class="flex justify-between items-center mb-6 flex-wrap gap-3">
                <h2 class="text-lg font-black text-slate-900 uppercase tracking-tight">Catálogo de Materias</h2>
                <div class="flex items-center gap-2 no-print">
                    <div class="flex items-center gap-2 bg-white rounded-xl border border-slate-200 shadow-sm px-4 py-2 shrink-0">
                        <span class="material-symbols-outlined text-slate-400 text-[16px]">school</span>
                        <select id="filter-materia-plan" onchange="horarios.renderMaterias()" class="text-xs font-bold text-slate-700 bg-transparent border-none outline-none">
                            <option value="">Todos los planes</option>
                        </select>
                    </div>
                    <label class="flex items-center gap-2 bg-white rounded-xl border border-slate-200 shadow-sm px-4 py-2 cursor-pointer no-print">
                        <input type="checkbox" id="filter-materia-activos" onchange="horarios.renderMaterias()" class="w-3.5 h-3.5 rounded text-blue-600">
                        <span class="text-xs font-bold text-slate-700">Solo planes activos</span>
                    </label>
                    <button onclick="horarios.openModal('bulk_materia')" class="flex items-center gap-2 bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 py-2 rounded-xl font-bold text-xs transition-all">
                        <span class="material-symbols-outlined text-[16px]">upload_file</span> Importar en bloque
                    </button>
                    <button onclick="horarios.openModal('materia')" class="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl font-bold text-xs transition-all shadow-sm shadow-blue-200">
                        <span class="material-symbols-outlined text-[16px]">add</span> Nueva Materia
                    </button>
                </div>
            </div>
                ...
                <table class="w-full text-sm">
                    <thead class="bg-slate-50 border-b border-slate-100">
                        <tr class="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                            ...
                            <th class="px-6 py-4 text-center">Hrs/Sem</th>
                            <th class="px-6 py-4 text-center">Tipo</th>
                            <th class="px-6 py-4 text-right">Acciones</th>
                        </tr>
                    </thead>
                    <tbody id="materias-tbody" class="divide-y divide-slate-50"></tbody>
                </table>
            </div>
        </div>

        <!-- ── ANÁLISIS ───────────────────────────────────────────────────── -->
        <div id="panel-analisis" class="hor-panel hidden">
            <div class="flex justify-between items-center mb-6">
                <div>
                    <h2 class="text-lg font-black text-slate-900 uppercase tracking-tight">Análisis de Asignación</h2>
                    <p class="text-xs text-slate-400 font-bold mt-0.5">Verifica qué materias están programadas para cada grupo, agrupado por cuatrimestre.</p>
                </div>
                <button onclick="horarios.loadAnalisis()" class="flex items-center gap-2 bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 py-2 rounded-xl font-bold text-xs transition-all">
        ...
        <!-- ── GRUPOS ─────────────────────────────────────────────────────── -->
        <div id="panel-grupos" class="hor-panel hidden">
            <div class="flex justify-between items-center mb-6 flex-wrap gap-3">
                <h2 class="text-lg font-black text-slate-900 uppercase tracking-tight">Gestión de Grupos</h2>
                <div class="flex gap-2">
                    <button onclick="horarios.syncGrupos()" class="flex items-center gap-2 bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 py-2 rounded-xl font-bold text-xs transition-all">
                        <span class="material-symbols-outlined text-[16px]">sync</span> Importar desde Asistencia
                    </button>
                    <button onclick="horarios.openModal('grupo')" class="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl font-bold text-xs transition-all shadow-sm shadow-blue-200">
                        <span class="material-symbols-outlined text-[16px]">group_add</span> Nuevo Grupo
                    </button>
                </div>
            </div>
            <p class="text-xs text-slate-400 font-bold mb-4">💡 "Importar desde Asistencia" sincroniza los grupos que ya existen en el sistema de listas.</p>
            <div id="grupos-list" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4"></div>
        </div>

        <!-- ── ALUMNOS ─────────────────────────────────────────────────────── -->
        <div id="panel-alumnos" class="hor-panel hidden">
            <div class="flex justify-between items-center mb-6 flex-wrap gap-4">
                <div>
                    <h2 class="text-lg font-black text-slate-900 uppercase tracking-tight mb-1">Carga de Alumnos</h2>
                    <p class="text-xs text-slate-400 font-bold">Selecciona un grupo o usa el importador inteligente para cargar listas completas.</p>
                </div>
                <div class="flex gap-2 no-print">
                    <button onclick="horarios.openModal('bulk_alumnos')" class="flex items-center gap-2 bg-slate-900 hover:bg-slate-800 text-white px-5 py-2.5 rounded-xl font-bold text-xs transition-all shadow-lg">
                        <span class="material-symbols-outlined text-[18px]">group_add</span> Importar en bloque
                    </button>
                    <button onclick="horarios.saveAlumnos()" class="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl font-bold text-xs transition-all shadow-lg shadow-blue-200">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
                    <label class="block text-xs font-black text-slate-600 mb-2 uppercase tracking-wide">Grupo destino</label>
                    <select id="alumnos-grupo-select" onchange="horarios.loadAlumnosGrupo()" class="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500 mb-4"></select>

                    <label class="block text-xs font-black text-slate-600 mb-2 uppercase tracking-wide">Pegar lista (Matrícula - Nombre, uno por línea)</label>
                    <textarea id="alumnos-textarea" rows="12" placeholder="20230001 - García Martínez Juan&#10;20230002 - Pérez López Ana&#10;..." class="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500 font-mono"></textarea>

                    <button onclick="horarios.saveAlumnos()" class="mt-4 w-full bg-blue-600 hover:bg-blue-700 text-white font-bold text-sm py-3 rounded-xl transition-all shadow-sm shadow-blue-200">
                ...
                <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
                    <div class="flex justify-between items-center mb-4">
                        <h3 class="text-sm font-black text-slate-800 uppercase">Alumnos actuales</h3>
                        <span id="alumnos-count" class="text-xs font-bold text-blue-600 bg-blue-50 px-2.5 py-0.5 rounded-full">0 alumnos</span>
                    </div>
                    <div id="alumnos-preview" class="space-y-2 max-h-[400px] overflow-y-auto">
                        <p class="text-slate-400 text-xs text-center py-8">Selecciona un grupo para ver sus alumnos</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <!-- ── AULAS ──────────────────────────────────────────────────────── -->
        <div id="panel-aulas" class="hor-panel hidden">
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-lg font-black text-slate-900 uppercase tracking-tight">Aulas y Espacios</h2>
                <button onclick="horarios.openModal('aula')" class="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl font-bold text-xs transition-all shadow-sm shadow-blue-200">
                    <span class="material-symbols-outlined text-[16px]">add</span> Nueva Aula
                </button>
            </div>
            <div id="aulas-list" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4"></div>
        </div>

        <!-- ── PROYECCIÓN ─────────────────────────────────────────────────── -->
        <div id="panel-proyeccion" class="hor-panel hidden">
            <div class="flex justify-between items-center mb-6">
                <div>
                    <h2 class="text-lg font-black text-slate-900 uppercase tracking-tight">Proyección de Carga Horaria</h2>
                    <div class="flex items-center gap-4 mt-1">
                        <p class="text-xs text-slate-400 font-bold" id="proyeccion-subtitle">Cargando periodo siguiente...</p>
                        <div class="flex items-center gap-2 bg-slate-50 px-2 py-1 rounded-lg border border-slate-100">
                            <label class="text-[10px] font-black text-slate-500 uppercase tracking-tight">Proyectar:</label>
                            <select id="proyeccion-depth" onchange="horarios.renderProyeccion()" class="bg-transparent text-[10px] font-black text-blue-600 focus:outline-none cursor-pointer">
                                <option value="1">1 Cuatrimestre</option>
                                <option value="2">2 Cuatrimestres</option>
                                <option value="3">3 Cuatrimestres</option>
                            </select>
                        </div>
                    </div>
                </div>
                <div class="flex gap-2">
                    <button id="btn-toggle-planning" onclick="state.isPlanning ? horarios.togglePlanning(false) : horarios.openWizard()" 
                        class="flex items-center gap-2 px-4 py-2 rounded-xl font-bold text-xs transition-all no-print shadow-sm bg-blue-50 text-blue-600 hover:bg-blue-100">
                        <span class="material-symbols-outlined text-[16px]">magic_button</span>
                        <span id="btn-toggle-planning-text">Asistente de Planeación</span>
                    </button>
                    <button id="btn-promote-groups" onclick="horarios.confirmPromoteGroups()" 
                        class="hidden flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-xl font-bold text-xs transition-all no-print shadow-sm shadow-emerald-200">
                        <span class="material-symbols-outlined text-[16px]">upgrade</span>
                        Aplicar Promoción Definitiva
                    </button>
                    <button onclick="horarios.renderProyeccion()" class="flex items-center gap-2 bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 py-2 rounded-xl font-bold text-xs transition-all">
                        <span class="material-symbols-outlined text-[16px]">refresh</span> Recalcular
                    </button>
                </div>
            </div>
            
            <div id="proyeccion-container" class="space-y-6">
                <div class="flex items-center justify-center py-12">
                    <p class="text-slate-400 text-sm font-bold flex items-center gap-2">
                        <span class="material-symbols-outlined animate-spin">sync</span>
                        Calculando proyecciones para el próximo cuatrimestre...
                    </p>
                </div>
            </div>
        </div>
...
<!-- ─── MODAL GENÉRICO ────────────────────────────────────────────────────── -->
<div id="hor-modal" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col" id="hor-modal-content">
        <div class="flex items-center justify-between p-6 border-b border-slate-100 shrink-0">
            <h3 class="font-black text-slate-900 text-lg" id="modal-title">Modal</h3>
            <button onclick="horarios.closeModal()" class="w-8 h-8 rounded-lg bg-slate-100 hover:bg-slate-200 flex items-center justify-center transition-colors">
        ...
        <div class="flex gap-3 justify-end p-6 pt-0 shrink-0">
            <button onclick="horarios.closeModal()" class="px-5 py-2.5 rounded-xl border border-slate-200 text-slate-600 font-bold text-sm hover:bg-slate-50 transition-colors">Cancelar</button>
            <button id="modal-save-btn" onclick="horarios.saveModal()" class="px-5 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-sm transition-colors shadow-sm shadow-blue-200">Guardar</button>
        </div>
    </div>
</div>

<!-- ─── FULLSCREEN WIZARD MODAL ───────────────────────────────────────────── -->
<div id="wizard-fullscreen-modal" class="fixed inset-0 z-[100] bg-slate-50 overflow-hidden flex-col hidden">
    <!-- Header -->
    <div class="bg-white border-b border-slate-200 px-8 py-5 flex items-center justify-between shadow-sm shrink-0">
        <div class="flex items-center gap-3">
            <span class="material-symbols-outlined text-blue-600 text-3xl">magic_button</span>
            <div>
                <h2 class="text-xl font-black text-slate-900 tracking-tight">Asistente de Planeación Prospectiva</h2>
                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Diseña el horario del próximo cuatrimestre</p>
            </div>
        </div>
        <button onclick="document.getElementById('wizard-fullscreen-modal').classList.add('hidden')" class="w-10 h-10 rounded-full bg-slate-100 hover:bg-slate-200 flex items-center justify-center transition-colors">
    ...
    <div class="flex-1 overflow-y-auto p-8 relative">
        <div class="max-w-6xl mx-auto" id="wizard-body-content">
            <!-- Pasos del Wizard inyectados por JS -->
        </div>
    </div>
</div>
...
<!-- ─── ESTILOS ────────────────────────────────────────────────────────────── -->
<style>
.hor-tab.active { color: #2563eb; border-bottom-color: #2563eb; }
.grid-container { grid-template-columns: 60px repeat(5, 1fr); min-width: 700px; }
.grid-header { padding:10px 8px; font-size:10px; font-weight:900; text-transform:uppercase; letter-spacing:.08em; color:#64748b; text-align:center; border-bottom:1px solid #f1f5f9; position:sticky; top:0; background:#f8fafc; z-index:2; }
.grid-time { padding:4px 8px; font-size:10px; font-weight:700; color:#94a3b8; text-align:center; border-right:1px solid #f1f5f9; background:#fff; position:sticky; left:0; z-index:1; border-bottom:1px solid #f8fafc; display:flex; align-items:center; justify-content:center; min-height:56px; }
.grid-cell { border-right:1px solid #f1f5f9; border-bottom:1px solid #f1f5f9; min-height:56px; position:relative; transition:background .15s; cursor:pointer; }
.grid-cell:hover { background:#f8fafc; }
.grid-cell.drag-over-ok  { background:#dcfce7; }
.grid-cell.drag-over-err { background:#fee2e2; }
.clase-block { position:absolute; border-radius:8px; padding:4px 6px; font-size:10px; font-weight:700; color:white; overflow:hidden; cursor:grab; z-index:1; box-shadow:0 2px 8px rgba(0,0,0,.15); transition:all .2s; user-select:none; }
.is-planning-grid .grid-cell { background: #fffcf0; border-color: #fef3c7; }
.is-planning-grid .grid-header { background: #fef3c7; color: #92400e; }
.is-planning-grid .grid-time { background: #fffbeb; color: #b45309; }
.is-planning-grid { border: 2px solid #fbbf24 !important; }
.clase-block:hover { opacity:1; transform:scale(1.05); z-index:10 !important; width:calc(100% - 4px) !important; left:2px !important; }
.clase-block.dragging { opacity:.4; }
@media print {
    body { background:white; }
    #horarios-app { margin:0 !important; padding:0 !important; }
    .no-print, .hor-tab, .bg-white.border-b, #panel-grid > .flex:first-child { display:none !important; }
    .grid-container { min-width:auto; width:100%; grid-template-columns: 50px repeat(5, 1fr) !important; font-size:8px; }
    .grid-cell { min-height:40px; }
    .grid-time { min-height:40px; font-size:8px; border-bottom:1px solid #eee; }
    .clase-block { padding:2px; box-shadow:none; border:1px solid rgba(0,0,0,0.1); color:#333 !important; }
    .clase-block * { color:#333 !important; }
}
</style>
